...
class Long_encoder(nn.Module):
    def __init__(self, c_in: int, long_context_window: int, target_window: int, m_patch_len: int, m_stride: int,
        ...
        super().__init__()
        # Patching
        self.m_patch_len = m_patch_len  # 补丁长度
        self.m_stride = m_stride  # 时间步长
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        z = z.unfold(dimension=-1, size=self.m_patch_len,
                     step=self.m_stride)  # z: [bs x nvars x m_patch_num x m_patch_len]
...
class Mamba_Encoder(nn.Module):
    def __init__(self, c_in, m_layers, d_model, d_ff, dropout, act, d_state, d_conv, m_patch_len):
        ...
        W_inp_len = m_patch_len  # 补丁长度
        self.W_P = nn.Linear(W_inp_len, d_model)
        self.m_layers = m_layers

        self.mamba_layers = nn.ModuleList(
            [Mamba_Encoder_Layer(d_model, d_ff, dropout, act, d_state, d_conv, m_patch_len) for i in range(m_layers)])
